 ...
 * ใช้ Sequelize + sqlite3 (cross-platform compatible)
 */
...
CREATE TABLE sync_logs (
  ...
  for (const row of branches) {
    ...
    await sequelize.query(
      ...
      { replacements: [generateUUID(), row.BRANCH_CODE, row.BRANCH_NAME, row.Branch_address, row.Branch_TEL] }
    );
  }
  ...
  for (const row of categories) {
    ...
    await sequelize.query(
      `INSERT INTO categories (id, code, name, isActive, createdAt, updatedAt) 
       VALUES (?, ?, ?, 1, datetime('now'), datetime('now'))`,
      { replacements: [generateUUID(), row.ICCAT_CODE, row.ICCAT_NAME] }
    );
  }
  ...
  for (const row of departments) {
    ...
    await sequelize.query(
      `INSERT INTO departments (id, code, name, isActive, createdAt, updatedAt) 
       VALUES (?, ?, ?, 1, datetime('now'), datetime('now'))`,
      { replacements: [generateUUID(), row.ICDEPT_CODE, row.ICDEPT_NAME] }
    );
  }
  ...
  for (const row of warehouses) {
    ...
    await sequelize.query(
      `INSERT INTO warehouses (id, code, name, location, isActive, createdAt, updatedAt) 
       VALUES (?, ?, ?, ?, 1, datetime('now'), datetime('now'))`,
      { replacements: [generateUUID(), row.WARE_CODE, row.WARE_NAME, row.WARE_LOCATION] }
    );
  }
  ...
  for (const row of products) {
    ...
    await sequelize.query(
      `INSERT INTO products (id, sku, name, nameEn, nameLo, category, description, isActive, createdAt, updatedAt) 
       VALUES (?, ?, ?, ?, ?, ?, ?, 1, datetime('now'), datetime('now'))`,
      { replacements: [productId, row.SKU_CODE, row.SKU_NAME, row.SKU_NAME_EN || row.SKU_NAME, row.SKU_NAME_LO || row.SKU_NAME, row.SKU_ICCAT, row.SKU_ICDEPT] }
    );
  }
  ...
  for (const row of goods) {
    ...
    await sequelize.query(
      `INSERT INTO product_units (id, productId, unitCode, unitName, unitNameEn, unitNameLo, conversionRate, barcode, isBaseUnit) 
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, 1)`,
      { replacements: [unitId, productId, row.GOODS_CODE, unitInfo.name, unitInfo.name, unitInfo.name, unitInfo.qty, row.GOODS_CODE] }
    );
    goodsCount++;
  }
  ...
  for (const row of prices) {
    ...
    await sequelize.query(
      `INSERT INTO product_prices (id, productId, unitId, priceLevel, price, effectiveDate) 
       VALUES (?, ?, ?, ?, ?, datetime('now'))`,
      { replacements: [generateUUID(), goodsInfo.productId, goodsInfo.unitId, priceLevel, price] }
    );
    priceCount++;
  }
  ...
  for (const row of customers) {
    ...
    await sequelize.query(
      `INSERT INTO customers (id, code, name, address, phone, email, priceLevel, creditLimit, isActive, createdAt, updatedAt) 
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, 1, datetime('now'), datetime('now'))`,
      { replacements: [generateUUID(), row.AR_CODE, row.AR_NAME, row.AR_ADD1 || null, row.AR_TEL || null, row.AR_EMAIL || null, row.AR_ARPRB || 1, row.AR_CREDIT || 0] }
    );
  }
  ...
  for (const row of users) {
    ...
    await sequelize.query(
      `INSERT INTO employees (id, code, name, username, password, role, isActive, createdAt, updatedAt) 
       VALUES (?, ?, ?, ?, ?, ?, 1, datetime('now'), datetime('now'))`,
      { replacements: [generateUUID(), row.USER_CODE, row.USER_NAME, row.USER_LOGIN, row.USER_PASSWORD, row.USER_LEVEL || 'CASHIER'] }
    );
  }
  ...
  for (const row of banks) {
    ...
    await sequelize.query(
      `INSERT INTO bank_accounts (id, code, bankName, accountNumber, accountName, branchName, isActive, createdAt, updatedAt) 
       VALUES (?, ?, ?, ?, ?, ?, 1, datetime('now'), datetime('now'))`,
      { replacements: [generateUUID(), row.BANK_CODE, row.BANK_NAME, row.BANK_ACCOUNT, row.BANK_ACC_NAME, row.BANK_BRANCH] }
    );
  }
